...
description: |
  ...
  输出时要主动强化前3秒钩子、极简口语表达、点赞/评论/收藏/转发/转化动机、结尾倒推结构，以及"但是/因此"式节奏推进，避免只生成平铺直叙的广告稿。
---
...
这个技能不只是写一条能读的脚本，而是要写一条有用户动作的脚本。每次生成前先判断脚本主攻什么动作：

- **点赞 = 共鸣**：用户有没有觉得「这不就是我吗」？
- **评论 = 讨论**：脚本里有没有争议点、选择题、真实问题，能让用户想留言？
- **收藏 = 有用**：有没有步骤、清单、参数、避坑、使用方法，能让用户想保存？
- **转发 = 利他**：有没有理由发给朋友、家人、同事或同类人群？
- **转化 = 行动**：用户看完是否知道下一步要点哪里、买什么、进哪里？

抖音脚本尤其要遵守四条：

1. **前3秒决定生死**，第一帧 + 第一句必须同时成立。静音看也要有信息，开声音听也要有钩子。
2. **简单到十岁小孩能懂**，短句、具体词、生活画面，不用专业黑话堆价值。
3. **先定结尾，再倒推开头**，先明确最后CTA和用户动作，再设计钩子、卖点顺序、情绪推进。
4. **用「但是 / 因此」做节奏**，脚本不能流水账。先给预期，再制造阻碍、反差或痛点，因此推出产品解决方案。
...
## 1. 钩子分析（前3秒）⭐ 最关键
前3秒 = 文案钩子 + 视觉钩子，两条线都要详细分析。额外判断：这个钩子主要触发什么用户动作，是点赞共鸣、评论讨论、收藏有用、转发利他，还是点击转化？
...
- BGM与画面的配合度
- 是否存在「但是 / 因此」式推进：预期→冲突/痛点→解决方案→结果
...
- 紧迫感营造手段
- 转化路径（视频→?→?）
- 结尾是否由最终动作倒推而来：用户看完到底该评论、收藏、转发、进直播间还是下单
...
## 8. 互动动机分析
- 这条视频主要靠什么驱动传播？点赞共鸣 / 评论讨论 / 收藏有用 / 转发利他 / 转化下单
- 各互动指标的强弱判断（1-5分）
- 最值得本品类借鉴的互动策略
...
2. **互动动机**：该品类主要靠点赞共鸣、评论讨论、收藏有用、转发利他，还是直接转化
3. **钩子公式**：最有效的钩子类型和句式模板，包含文案钩子 + 第一帧视觉钩子
4. **结尾倒推**：这类脚本的最终动作是什么，前面如何为CTA铺垫
5. **最佳分镜结构**：适合该品类的标准分镜流程
6. **节奏模板**：推荐的节奏设计方案，明确预期→但是→因此→结果的推进线
7. **转化路径**：最优的CTA策略
8. **风险提示**：需要避免的合规问题
...
- 简单到十岁小孩能懂：不用专业黑话，所有卖点都转成生活场景
- 具体数据锚点：用具体数字，不用「很多」「大量」
- 感官描述：调动视觉/触觉/味觉/嗅觉
- 情感触发：身份认同、焦虑激活、场景共鸣
- 互动触发：明确这条脚本靠点赞、评论、收藏、转发还是转化，不要只讲卖点
- 品牌背书简化：一句话带过
- 价格锚定+紧迫感
- 但是/因此推进：每个卖点前后要有痛点、反差、因果，不要流水账
...
### Part A：完整脚本
- 先确定结尾CTA和用户动作，再倒推开头和中间卖点顺序
- 开头3秒必须有强钩子（参考分析中最有效的钩子类型）
- 第一帧静音也要能看懂核心冲突或利益点
- 中间展示核心卖点（最多3个，多了观众记不住）
- 每个卖点用「痛点/预期 → 但是 → 因此 → 结果」推进，不要平铺直叙
- 结尾有明确CTA，并解释这个CTA服务于哪种动作：评论/收藏/转发/进直播间/下单
- 全文口语化，像朋友推荐
- 不使用绝对化用语，符合抖音广告审核规范
- 标注每段预估时长
...
## 千川素材特殊要求
- 先确定最终转化动作：点击/进直播间/下单，再倒推前3秒钩子
- 前3秒必须有强钩子（数据型或痛点型，不要用问句开头）
- 信息密集，卖点直给，但必须简单到非专业用户一听就懂
- CTA要硬：明确引导点击/进直播间/下单
- 卖点表达要有「痛点 → 反差 → 解决 → 结果」推进，避免连续堆功能
...
#### 检查5：互动与转化检查
- 这条脚本主攻的动作是否清楚：点赞/评论/收藏/转发/进直播间/下单
- 如果主攻收藏，是否有步骤、清单、参数、避坑、方法
- 如果主攻评论，是否有真实问题、争议点、选择题
- 如果主攻转发，是否有明确人群和分享理由
- 如果主攻转化，CTA是否足够明确，且前面有足够铺垫
