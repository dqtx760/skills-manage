...
在HKR之后，再做一次「四互动」判断。流量不是玄学，而是用户有没有理由互动：
- **点赞 = 共鸣**，这篇文章有没有替读者说出一句心里话？
- **评论 = 讨论**，有没有争议、提问、选择题，或者能让人忍不住补充的口子？
- **收藏 = 有用**，有没有步骤、清单、模板、参数、避坑、案例拆解这种可保存模块？
- **转发 = 利他**，读者有没有理由把它发给朋友、同事、群友？

不要求每篇四项全满，但至少要明确主攻哪两项。方法论、教程、工具文优先做「收藏 + 转发」，观点文和现象文优先做「共鸣 + 评论」。如果四项都弱，先别急着写，应该调整选题或角度。
...
**观点解读型**：针对一个事件、文章或现象发表自己的解读和判断。核心是"我读完了，跟你说说我觉得最重要的几点"。写法要点：
- 用序号标注每个观点（01.、02.、03. ...），每个观点用 `### 0X.` 加三级标题
- 观点数量控制在 3 到 7 个，宁精勿滥，每个观点要有具体数据或细节支撑
- 正文精炼，每个观点 100 到 200 字，全文正文（不含开头结尾尾部）控制在 500 到 1000 字
- 结尾用一句有记忆点的话收束，不拖泥带水

```
【开头】感性切入，抛出事件，制造冲突感或好奇心（2-3 句）
  ↓
### 01. 第一个观点
  - 核心判断 + 关键数据/细节支撑
  - 一句口语化点评或情绪反应
...
### 03. 第N个观点
  - ...
  ↓
【收尾】一句有记忆点的话，可以是金句反转、文化升维或留白
```

### 风格内核

**简单到十岁小孩能懂**：复杂不等于专业，用户愿意读下去的前提是马上能明白你在说什么。写完每个核心观点都问一句，一个小学五年级的小孩能不能听懂？如果不能，就继续把术语换成画面，把抽象判断换成具体场景，把长句拆短。降低的是理解门槛，不是内容价值。

**先定结尾，再倒推正文**：不要想到哪写到哪。动笔前先写出最后一句，明确读者看完后要记住什么、要产生什么动作，是点赞、评论、收藏、转发，还是点进下一篇。然后倒推开头钩子、中间转折、案例铺垫和结尾callback。这样文章不会散，读者会被一根线牵到最后。

**开头前三秒意识**：公众号不是短视频，但开头同样只有极短的耐心窗口。第一屏要给出强画面、强结果、强冲突或强疑问，最好让读者一眼知道「这事跟我有关，而且后面有东西」。不要用"今天聊聊"这种慢启动。

**但是 / 因此推进**：平铺直叙最容易无聊。写故事、案例、方法论时，多用内在的转折和因果推进，先建立一个预期，然后用「但是」制造阻碍，再用「因此」推动下一步。不是机械堆这两个词，而是让每一段都有变化、冲突和结果。
...
**真实人格优先**：AI可以帮你找资料、搭结构、改句子，但文章里最值钱的是活人感。真人经历、真实判断、真实踩坑、不完美的表达、现场反应，这些才是读者信任的来源。不要把文章写成一个完美但没有体温的说明书。
...
**方法论文章的结构原则**：当文章类型是"教你怎么做"时，必须确保每一节读者读完后手里有一个可以今天就执行的动作。好的结构是"观点→案例/理论支撑→所以具体怎么做→坦诚说明学习曲线和失败点"。同时至少放入一个「可收藏模块」，比如检查表、步骤清单、模板、参数、避坑列表、复盘表格。收藏不是附带数据，收藏是知识类内容能不能长尾传播的关键。
...
2. **过度结构化**：不用bullet point罗列观点，不大量加粗。卡兹克的文章绝大多数时候没有小标题，从头到尾一口气顺下来，靠节奏和转场自然推进。例外有二：一是"N条心得/方法"这种本身就是独立条目的方法论文章，可以用数字编号（1、2、3），但也不是正式的markdown标题，就是文中的数字；二是观点解读型文章，用 `### 01.`、`### 02.` 这种带序号的三级标题标注每个观点。如果不是这两种结构，就不要加小标题，用口语化的转场句（"说到这个""回到xxx这块"）来衔接板块
3. **标点禁令**：
   - 不使用冒号"："，用逗号代替
   - 不使用破折号"——"
...
```
【动笔前】先写最后一句话，明确这篇文章最终要让读者记住什么、做什么
  ↓
【开头】感性切入，从一个具体事件/场景开始，前三秒给出强画面、强结果、强冲突或强疑问
  ↓
【背景铺垫】简要科普，让非专业读者也能看懂，最好小学五年级也能听明白，但科普方式是聊天式的
  ↓
### 第一个核心板块标题
  - 一个明确观点
  - 至少一个具体场景/人物/对话支撑
...
【升华】从具体事件拉到更大的文化/哲学/历史参照物
  ...
  ↓
【收尾】几种常见收法，选最合适的一种。结尾3到5句话要有记忆点，最好能触发点赞、评论、收藏、转发中的至少一个动作：
  - 引用收尾：用别人的一句话作结（"比比说：磨平一些信息差"）
  - 哲思余韵：一个短句留白（"时间。流逝的本身。"）
  - 行动呼吁：鼓励读者去做一件事
  - 信念宣言：表达对未来的坚信
  - 回环呼应：回到开头的意象，但视角已经不同
  - 可收藏收尾：给出一张清单、一个模板、一个判断标准，让读者有保存理由
  - 讨论收尾：留下一个真问题，不是客套互动，而是真的值得评论区聊
  ↓
【固定尾部】
...
写完文章后，按以下规则自动生成和插入配图：

1. **封面图**：引言段落结束后、第一个 `###` 标题之前，插入一张 16:9 封面图（使用 `article-cover-16x9` 技能）
2. **正文配图**：每个核心观点板块的关键句之后，插入一张配图（使用 `baoyu-article-illustrator` 技能）
3. **配图节奏**：5 个板块约 3-5 张正文配图，每个板块最多 1 张，不打断阅读节奏
4. **图片格式**：全部上传 Gitee 图床（PicGo），使用 `![]()` 嵌入

**插入位置模板**：
```
引言最后一段（点题句）
↓
![封面图](图床URL)        ← 16:9 封面，引言与正文之间
↓
### 01. 第一个观点
  ...正文...
  关键金句。
...
文章结尾的 `\>>>**延伸阅读**` 板块用于添加往期相关文章，目的是交叉导流。

**核心规则**：
- ✅ **往期公众号文章（`mp.weixin.qq.com`）加超链接**——微信生态内，可正常跳转
- ⛔ **外部链接不加超链接**——URL 放在标题下一行，纯文本展示，方便读者复制
- 板块标题用 `\>>>**延伸阅读**` 或 `\>>>**相关阅读**`

**操作流程**：

1. **首先问用户**：写完文章后，先问用户「有没有想推荐的往期文章？」如果用户指定了，直接使用。
2. **用户没指定时，自动匹配**：去往期文章合集 `D:\project2026\fuwari\src\content\Yoke\微信公众号文章合集.md` 中，根据当前文章的主题和关键词，挑选 2-3 篇最相关的往期文章。
3. **挑选原则**：
   - 优先从「观点」分类中找，找不到再扩展到其他分类
   - 主题相关度 > 发布时间新旧
   - 宁缺毋滥，如果实在没有高度相关的，只放 1 篇或直接留空
4. **如果没有合集文件或找不到相关文章**：标题保留但下方留空，或直接省略该板块。
5. **如果是调研类文章**：在延伸阅读下方额外列出参考来源（纯文字，不加链接）。

**格式示例**：
```
\>>>**延伸阅读**

- [普通人在AI时代的破局之路](https://mp.weixin.qq.com/s/xxx)
- [用 AI 两年，我劝你别太依赖它](https://mp.weixin.qq.com/s/xxx)
- 某外部参考文章
...
- 需要插图的位置标注"图片"即可
- **使用三级标题（###）划分主要板块**。每个核心论点用一个 ### 标题标记，让读者一眼看清文章结构。板块之间再加口语化转场句自然衔接（"说到这个""回到xxx这块""顺着上面的再聊聊"）
...
**L2-2 节奏与结构检查**
- 是否先明确了结尾，再倒推开头和中间结构？
- 开头第一屏是否有强画面、强结果、强冲突或强疑问？
- 核心观点是否简单到非专业读者也能立刻听懂？
- 中间是否有「但是 / 因此」式的转折和因果推进，而不是流水账？
...
- 有没有空泛的观点只有论断没有例证？
- 这篇文章主攻的互动指标是什么，共鸣、讨论、收藏、转发里至少有哪两项成立？
- 如果是知识/教程/方法论内容，是否有清单、步骤、模板、参数、避坑、案例拆解这类可收藏模块？
...
- 观点解读型 → 观点是否用 `### 0X.` 序号标注？是否控制在 3 到 7 个？每个观点是否有数据或细节支撑？正文是否精炼（500-1000 字）？
